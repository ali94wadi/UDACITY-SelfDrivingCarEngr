...
Eigen::Matrix4d NDT(PointCloudT::Ptr mapCloud, PointCloudT::Ptr source, Pose startingPose) {
    ...
    int iterations = 100; // Setting max number of registration iterations.
...
int main(int argc, char *argv[]) {
	...
	int n_scans = 0;// Declare the variable number of scans.
	...
	while (!viewer->wasStopped())
  	{
		...
		if(!new_scan){
			...
			vg.setInputCloud(scanCloud);// Set the input cloud
			double filterRes = 0.75; //0.1; //0.5;// Declare the filter resolution
			// Set the leaf size of the voxel grid.
			vg.setLeafSize(filterRes, filterRes, filterRes);
			// Declare the cloud filtered.
			typename pcl::PointCloud<PointT>::Ptr cloudFiltered (new pcl::PointCloud<PointT>);
			vg.filter(*cloudFiltered);// Filter the cloud points.
