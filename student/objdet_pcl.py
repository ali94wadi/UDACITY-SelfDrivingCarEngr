...
def show_pcl(pcl):
    ...
    vis_pcl.create_window(window_name='Open3D', width=1920, height=1080, left=0, top=0, visible=True)
    global click
    click = True
...
def show_range_image(frame, lidar_name):
    ...
    img_range_intensity = np.vstack((ri_range,ri_intensity)).astype(np.uint8) # remove after implementing all steps
    #######
    ####### ID_S1_EX1 END #######     
...
def bev_from_pcl(lidar_pcl, configs):
    ...
    lidar_pcl_int = lidar_pcl_cpy # MORE WORK MIGHT BE NEEDED
    ...
    _, idx_int_unique, counts  = np.unique(lidar_top_pcl[:, 0:2], axis=0, return_index=True, return_counts=True)
    lidar_top_pcl = lidar_top_pcl[idx_int_unique]
